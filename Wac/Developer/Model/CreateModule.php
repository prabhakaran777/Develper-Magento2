...
class CreateModule
{
    ...
    public function execute($vendorName,$moduleName){
        ...
        try {
            ...
            $this->creteComposerFile($moduleFullName,$moduleDirectory);
    ...
    public function creteComposerFile($moduleName,$moduleDirectory){

        $fileName='composer.json';

        $moduleSplit=explode('_',$moduleName);
        $vendor=$moduleSplit[0];
        ...
        $templateData = str_replace('%vendor%', $vendor, $template);

        $templateData = str_replace('%module%', $module, $templateData);

        $vendorSmall=strtolower($vendor);
        $moduleSmall=strtolower($module);

        $templateData = str_replace('%vendor-small%', $vendorSmall, $templateData);

        $templateData = str_replace('%module-small%', $moduleSmall, $templateData);
    ...
    {
        $read = $this->readFactory->create(__DIR__ . '/');
        $content = $read->readFile('composer.json.dist');
        return $content;
    }
